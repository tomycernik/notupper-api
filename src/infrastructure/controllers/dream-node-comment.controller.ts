  ...
  async getComments(req: Request, res: Response) {
    try {
      ...
      if (!nodeId)
        return res
          .status(400)
          .json({ success: false, message: "Falta nodeId" });
      const currentUserId = (req as any).userId;
      const comments = await this.commentService.getCommentsByNodeWithUser(
        nodeId
      );

      const { likeCount, likedByMe } =
        await this.dreamNodeService.getNodeLikeInfo(nodeId, currentUserId);
    ...
    try {
      ...
      if (dreamNode.privacy !== "Publico") {
        return res.status(403).json({
          success: false,
          message:
            "Este sueño no es público. Los comentarios solo están disponibles para sueños públicos.",
        });
      }
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            success: false,
            message: "Faltan datos o el comentario es inválido",
          });
      }
      const commentWithUser = await this.commentService.addCommentWithUser(
        nodeId,
        profileIdFrom,
        content.trim()
      );
      const profileIdTo = await this.userService.getUserIdByDreamNodeId(nodeId);
      const userNameFrom = await this.userService.getUserNameById(
        profileIdFrom
      );
      ...
      if (!dreamNode) {
        return res
          .status(400)
          .json({ success: false, message: "No existe Dream Node" });
      }
